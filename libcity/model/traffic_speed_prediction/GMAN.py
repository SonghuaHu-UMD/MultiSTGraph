...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x, ste):
        ...
        # (K*batch_size, num_step, num_nodes, d) multi-head attentions
...
class GMAN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.bn = True
        self.bn_decay = 0.1
        self.input_dim_time_excl = self.feature_dim - 8 if self.add_day_in_week else self.feature_dim - 1
        # define the model structure
        self.input_fc = FC(input_dims=self.input_dim_time_excl, units=[self.D, self.D], activations=[nn.ReLU, None],
                           bn=self.bn, bn_decay=self.bn_decay, device=self.device)
    ...
    def forward(self, batch):
        ...
        index_s = self.data_feature.get('feature_dim') - self.data_feature.get('ext_dim')
        index_e = index_s + 8 if self.add_day_in_week else index_s + 1
        x = torch.cat((x_all[:, :, :, 0:index_s], x_all[:, :, :, index_e:]), dim=-1)
        SE = torch.from_numpy(self.SE).to(device=self.device)
        TE = torch.cat((x_all[:, :, :, index_s:index_e], y_all[:, :, :, index_s:index_e]), dim=1)
        _timeofday = TE[:, :, :, 0:1]
