...
class ATGRUEncoder(nn.Module):
    def __init__(self, config, feature_final, adj_max):
        ...
        self.adj_mx = adj_max
        assert self.num_layers >= 1, 'At least one layer in the Encoder'
...
class ATGRUDecoderAtt(nn.Module):
    def __init__(self, config, adj_max):
        super(ATGRUDecoderAtt, self).__init__()
        self.num_nodes = config['num_nodes']
        self.hidden_dim = config.get('rnn_units', 64)
        self.num_layers = config.get('num_layers', 2)
        self.embed_dim = config.get('embed_dim', 10)
        ...
        assert self.num_layers >= 1, 'At least one layer in the Decoder'
...
class ATGRUDecoder(nn.Module):
    ...
    def forward(self, y_prev, y_labels, decoder_hidden, encoder_output, node_embeddings, nodevec1, nodevec2):
        ...
        for t in range(self.output_window):
            ...
            for layer_num, dcgru_layer in enumerate(self.atgru_cells):
                state = decoder_hidden[layer_num]
                decoder_input = dcgru_layer(decoder_input, state, node_embeddings, nodevec1, nodevec2)
                hidden_states.append(state)
            decoder_hidden = hidden_states
            decoder_output = self.projection_layer(decoder_input)
            decoder_input = torch.cat((decoder_output, y_labels[:, t, :, 1:4]), dim=-1)
            outputs.append(decoder_output)
            if self.training and self.use_curriculum_learning:
                c = np.random.uniform(0, 1)
...
class MultiATGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        if self.atten:
            self.decoder = ATGRUDecoderAtt(config, self.adj_max)
        else:
            self.decoder = ATGRUDecoder(config, self.adj_max)
    ...
    def forward(self, batch):
        ...
        if self.s2s:
            ...
            # CNN based predictor
            output = F.dropout(encoder_output, p=0.1, training=self.training)
