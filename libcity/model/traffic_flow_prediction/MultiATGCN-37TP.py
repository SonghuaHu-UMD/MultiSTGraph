...
class AVWDCRNN(nn.Module):
    def __init__(self, config):
        ...
        for _ in range(0, self.poi_dim):
            self.dcrnn_cells.append(AGCRNCell(self.num_nodes, self.feature_dim - self.poi_dim + 1,
        ...
        self.end_conv = nn.Conv3d(self.input_window, self.output_window, kernel_size=(1, 1, self.hidden_dim), bias=True)
        # self.fusionlayer = FusionLayer(self.output_window, self.num_nodes, self.poi_dim)
    ...
    def forward(self, x, init_state, node_embeddings):
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):
                for j in range(self.poi_dim):  # 7 POI
                    if i == 0:
                        state = self.dcrnn_cells[i * self.poi_dim + j](
                            torch.cat((current_inputs[:, t, :, j:j + 1], current_inputs[:, t, :, self.poi_dim:]),
                                      dim=-1), state, node_embeddings[j])
                    else:
                        state = self.dcrnn_cells[i * self.poi_dim + j](current_inputs[:, :, :, t, j], state,
                                                                       node_embeddings[j])
                    inner_states.append(state)
            current_inputs = torch.stack(inner_states, dim=-1)
            current_inputs = torch.reshape(current_inputs,
                                           (-1, self.num_nodes, self.hidden_dim, seq_length, self.poi_dim))
        # current_inputs: the outputs of last layer: (B, T, N, hidden_dim)
        # output_hidden: the last state for each layer: (num_layers, B, N, hidden_dim)
        # last_state: (B, N, hidden_dim)
        # output_hidden = self.ln(output_hidden[-1])
        output_hidden = self.end_conv(current_inputs.permute(0, 3, 1, 4, 2))  # B, T*C, N, 1
        # output_hidden = self.end_conv(output_hidden[-1].unsqueeze(dim=1))
        # output_hidden = self.fusionlayer(torch.squeeze(output_hidden))
        return torch.squeeze(output_hidden)
...
class MultiATGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.poi_dim = config.get('poi_dim', 1)

        self.nembeds = nn.ParameterList(
            [nn.Parameter(torch.randn(self.num_nodes, self.embed_dim), requires_grad=True) for i in
             range(self.poi_dim)])
        self.encoder_time = AVWDCRNN(config)
        # self.ln = nn.LayerNorm(self.hidden_dim)
        ...
        self.weight_t1 = nn.Parameter(torch.FloatTensor(1, self.len_closeness, 1, self.feature_dim))
        self.weight_t2 = nn.Parameter(torch.FloatTensor(1, self.len_period, 1, self.feature_dim))
        self.weight_t3 = nn.Parameter(torch.FloatTensor(1, self.len_trend, 1, self.feature_dim))
    ...
    def forward(self, batch):
        ...
        source = batch['X']
        output = 0.0
        init_state = self.encoder_time.init_hidden(source.shape[0])
        ...
        results = self.encoder_time(output, init_state, self.nembeds)
