    ...
    def forward(self, x, init_state, node_embeddings, nodevec1, nodevec2):
        ...
        for i in range(self.num_layers):
            ...
            for t in range(seq_length):
                state = self.dcrnn_cells[i](current_inputs[:, t, :, :], state, node_embeddings, nodevec1, nodevec2)
...
class MultiATGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        # self.weight_s1 = nn.Parameter(torch.FloatTensor(1, self.len_trend, self.num_nodes, self.feature_used + 3))
        # self.weight_v1 = nn.Parameter(torch.FloatTensor(1, 1, self.num_nodes, self.feature_used + 3))
    ...
    def forward(self, batch):
        ...
        # if self.static is not None:
        #     u, s, v = torch.pca_lowrank(self.static, q=10)
        #     initemb = torch.matmul(self.static, v)
        ...
        # weight before GRU
        # output = output * self.weight_v1
        # GRU encoder
        init_state = self.encoder.init_hidden(source.shape[0])
        output, _ = self.encoder(output, init_state, self.node_embeddings, self.nodevec1,
                                 self.nodevec2)  # B, T, N, hidden
