                ...
                for j in range(self.poi_dim):  # 7 POI
                    if i == 0:
                        state = self.dcrnn_cells[i](
                            torch.cat((current_inputs[:, t, :, j:j + 1], current_inputs[:, t, :, 7:]), dim=-1), state,
                            node_embeddings)
                    else:
                        state = self.dcrnn_cells[i](current_inputs[:, :, :, t, j], state, node_embeddings)
...
class MultiATGCN(AbstractTrafficStateModel):
    def __init__(self, config, data_feature):
        ...
        self.nembed = nn.Parameter(torch.randn(self.num_nodes, self.embed_dim), requires_grad=True)
    ...
    def forward(self, batch):
        ...
        if self.len_closeness > 0:
            ...
            output_hours = self.encoder_time(source[:, begin_index:end_index, :, :], init_state, self.nembed)
            output += output_hours
        # if self.len_period > 0:
        #     begin_index = self.len_closeness
        #     end_index = begin_index + self.len_period
        #     output_days = self.encoder_time(source[:, begin_index:end_index, :, :], init_state, self.nembed)
        #     output += output_days
        # if self.len_trend > 0:
        #     begin_index = self.len_closeness + self.len_period
        #     end_index = begin_index + self.len_trend
        #     output_weeks = self.encoder_time(source[:, begin_index:end_index, :, :], init_state, self.nembed)
        #     output += output_weeks
        return output
    ...
    def calculate_loss(self, batch):
        ...
        return loss.masked_mae_torch(y_predicted.sum(-1, keepdims=True), y_true.sum(-1, keepdims=True), 0)
