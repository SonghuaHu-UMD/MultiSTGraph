...
class MultiATGCN(AbstractTrafficStateModel):
    ...
    def forward(self, batch):
        ...
        if self.len_closeness > 0:
            ...
            output_hours = self.encoder_close(output_hours, init_state, self.node_emb, self.node_vec1, self.node_vec2,
                                              self.supports)
            output += output_hours
        if self.len_period > 0:
            ...
            output_days = self.encoder_period(output_days, init_state, self.node_emb, self.node_vec1, self.node_vec2,
                                              self.supports)
            output += output_days
        if self.len_trend > 0:
            ...
            output_weeks = self.add_future_known(output_weeks, batch)
            output_weeks = self.encoder_trend(output_weeks, init_state, self.node_emb, self.node_vec1, self.node_vec2,
                                              self.supports)
            output += output_weeks
        return output
